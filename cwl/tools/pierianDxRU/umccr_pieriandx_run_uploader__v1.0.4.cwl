    ...
    listing: 
      ...
      - entryname: run_pieriandx_run_uploader.sh
        entry: |-
          ...
          mkdir -p \$path_msi
          cp $(inputs.msi_json[0].path) \$path_msi

          path_tmb=TSO500_ctDNA_output/Logs_Intermediates/Tmb/$(inputs.sample_id)
          mkdir -p \$path_tmb
          cp $(inputs.tmb_json[0].path) \$path_tmb

          path_results=TSO500_ctDNA_output/Results/$(inputs.sample_id)
          mkdir -p \$path_results
          cp $(inputs.results_file_list[0].path) $(inputs.results_file_list[1].path) $(inputs.results_file_list[2].path) \$path_results
